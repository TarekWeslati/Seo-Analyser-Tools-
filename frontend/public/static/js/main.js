document.addEventListener('DOMContentLoaded', () => {
    ...
    rewriteSeoButton.addEventListener('click', async () => {
        ...
        try {
            ...
            if (rewriteSeoOutput) rewriteSeoOutput.innerHTML = `<p class="text-red-600">${error.message}</p>`; // Null check
        } finally {
    ...
    refineContentButton.addEventListener('click', async () => {
        ...
        try {
            ...
            if (refineContentOutput) refineContentOutput.innerHTML = `<p class="text-red-600">${error.message}</p>`; // Null check
        } finally {
